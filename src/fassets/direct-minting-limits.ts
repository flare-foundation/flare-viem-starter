// Run: pnpm run script src/fassets/direct-minting-limits.ts
// Optional: MINT_PREFLIGHT_XRP=50 pnpm run script src/fassets/direct-minting-limits.ts
...
function computeWindowExecutionAllowedAt({
  ...
  return (
    effectiveStart + (windowSizeSeconds * mintedAfter) / limitUBA
  );
}
...
function computeDirectMintingExecutionAllowedAt({
  ...
  return { executionAllowedAt, delayed, delayReasons, hourlyAt, dailyAt, largeAt };
}
